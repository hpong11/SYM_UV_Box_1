...
void loop() {
  ...
  if(MainMode == ModeSetup){
    if((CurrentSegBlinkMill + SegmentBlinkTime <= millis()) && (SegBlinkMode == 0)){
      CurrentSegBlinkMill = millis();
      SetupModeDisp(SegBlinkMode);
      Seven_Segment_Display();
      SegBlinkMode = 1;
    }
    else if((CurrentSegBlinkMill + SegmentBlinkTime <= millis()) && (SegBlinkMode == 1)){
      CurrentSegBlinkMill = millis();
      SetupModeDisp(SetupMode);
      Seven_Segment_Display();
      SegBlinkMode = 0;
    }
    if((digitalRead(SW_S) == 0) && (millis()-CurrentSW_S>=500)){
      CurrentSW_S = millis();
      if(SetupMode==Ope) SetupMode = Tim;
      else if(SetupMode==Tim) SetupMode = Ope;
      CurrentSegBlinkMill = millis();
      SetupModeDisp(SetupMode);
      Seven_Segment_Display();
      SegBlinkMode = 0;
